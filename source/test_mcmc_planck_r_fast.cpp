...
std::string
TestMCMCPlanckRFast::name() const
{
    return std::string("MCMC FAST PLANCK LIKELIHOOD WITH TENSOR TESTER");
}

unsigned int
TestMCMCPlanckRFast::numberOfSubtests() const
{
    return 1;
}

void
TestMCMCPlanckRFast::runSubTest(unsigned int i, double& res, double& expected, std::string& subTestName)
{
    ...
    std::string errorLogRoot = "slow_test_files/mcmc_planck_r_fast_error_log";
    planckLike.logError(errorLogRoot.c_str());
    std::string root = "slow_test_files/mcmc_planck_r_fast_test";
    MetropolisHastings mh(21, planckLike, root, std::time(0), true);
    ...
    mh.setParam(6, "r", 0.0, 0.5, 0.1, 0.02, 0.002);
    ...
    std::ofstream outParamLimits("slow_test_files/mcmc_planck_r_fast_param_limits.txt");
    for(int i = 0; i < 21; ++i)
    {
        const std::string& paramName = mh.getParamName(i);
        std::stringstream fileName;
        fileName << "slow_test_files/mcmc_planck_r_fast_" << paramName << ".txt";
